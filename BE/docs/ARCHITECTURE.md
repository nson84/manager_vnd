...
├── user/                    # User CRUD + profile (chủ/nhân viên đăng nhập)
├── company/                 # Company CRUD
├── role/                    # Role CRUD + assign permissions
├── permission/              # Permission CRUD + RBAC mapping
├── customer/                # Khách mua chịu
├── worker/                  # Công nhân / thợ (không login)
├── expense/                 # Loại chi + phiếu chi cửa hàng
├── debt/                    # Sổ công nợ khách + ứng lương thợ
├── wage/                    # Ghi công thợ
├── payslip/                 # Quyết toán / trả lương
└── cashbook/                # Sổ quỹ — nguồn thống kê chi tiêu
```
...
expense-category  ← (no dependencies)
customer          ← (no dependencies)
worker            ← (no dependencies)

debt       ← customer, worker, user
wage       ← worker, user
expense    ← expense-category, user
cashbook   ← expense-category, user   (dòng sổ do service khác ghi)

payslip    ← worker, wage, debt, cashbook, user
```
...
- Business features must not depend on `auth` package (use `SecurityUtil` / `User`)
- `payslip` is the only feature that orchestrates wage + debt + cash in one transaction
- **No circular dependencies allowed**
- Domain money rules: `BE/docs/decisions/003-shop-ledger-payroll.md`
- Schema: `BE/docs/DATABASE.md`
---
